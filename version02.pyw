...
import version02_layout as layout
...
class GAME():
    ...
    def drawLines(self):  # Iterate over each cell, jumping cellSize n each time, with 1 pixel for padding on the outside
        for indexX in range(0, self.x + 1, self.cellSize):
            pygame.draw.line(self.screen, (255, 255, 255), (indexX, 0), (indexX, self.y), 1)
        for indexY in range(0, self.y + 1, self.cellSize):
            pygame.draw.line(self.screen, (255, 255, 255), (0, indexY), (self.x, indexY), 1)


listB, listS, fps, frequency, cellSize, x, y, padding, probability = layout.launchRules()  # Start the UI as defined in the layout file and assign the values return

state = "place"  # Start in place mode
counter = frequency

pygame.init()
screen = pygame.display.set_mode((x * cellSize + 1, y * cellSize + 16 + 2))  # Set the screen size to be big enough for the number of cells (x * cellSize by y * cellSize) and 1 pixel for padding on the outside
...
button = pygame.Rect(0, y * cellSize, x * cellSize + 1, y * cellSize + 16)  # Define rect for button

for indexC in range(0 + padding, x - padding):
    for indexR in range(0 + padding, y - padding):
        board[indexR][indexC] = random.randint(0, probability) // probability  # Each cell has a 1 in probability n chance of starting alive
